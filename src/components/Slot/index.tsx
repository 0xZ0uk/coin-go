...
import { durstenfeldShuffle, seededDurstenfeldShuffle } from "@/utils/array";
 ...
 */
	...
	const [privateSeed, setPrivateSeed] = React.useState(0);
	const [publicSeed, setPublicSeed] = React.useState(0);

	React.useEffect(() => {
		const privateRandom = squares(privateSeed);
		const publicRandom = squares(publicSeed);
		setPrivateSeed(privateRandom);
		setPublicSeed(publicRandom);
